...
def helpMessage() {
    ...
    References                          If not specified in the configuration file or you wish to overwrite any of the references
      ...
      --save_reference [bool]           If generated by the pipeline save the bwa indices in the results directory (Default: false)
    ...
    Variant calling
      ...
      --ivar_trim_offset [int]          This option sets the '-x' parameter in 'ivar trim' so that reads that occur at the specified offset positions relative to primer positions will also be trimmed.
      --read_trim_end_offset [int]          This option is recommended for swift kit. If set it will trim so many bases from the end of the read. Default disabled
...
if (!params.skip_adapter_trimming) {
    process FASTP {
        ...
        offset_trim_arg_fastp = params.read_trim_end_offset ? "--trim_front1 ${params.read_trim_end_offset} --trim_tail1 ${params.read_trim_end_offset}" : ''
        """
        ...
        fastp \\
            ...
            --html ${sample}.fastp.html $offset_trim_arg_fastp\\
            2> ${sample}.fastp.log
...
if (!params.skip_kraken2 ) {
    process KRAKEN2AMP {
        ...
        output:
        tuple val(sample), val(single_end), path("*.viral*") into ch_fastp_bwa
...
/*
 * PREPROCESSING: Build Bwa index for viral genome
 */
process BWA_INDEX {
    tag "$fasta"
    label 'process_medium'
    ...
    bwa index \\
        -p $index_base \\
        $fasta
    mkdir BwaIndex && mv ${index_base}* BwaIndex
    """
}
...
process BWA_MAP{
    ...
    input:
    tuple val(sample), val(single_end), path(reads) from ch_fastp_bwa
    path index from ch_index

    output:
    tuple val(sample), val(single_end), path("*.bam") into ch_bwa_bam
    path "*.log" into ch_bwa_mqc

    script:
    input_reads = single_end ? "$reads" : "${reads[0]} ${reads[1]}"
    ...
    bwa mem \\
     ...
     samtools stats ${sample}.bam > ${sample}.bwa.log
    """
}
...
process SORT_BAM {
    ...
    input:
    tuple val(sample), val(single_end), path(bam) from ch_bwa_bam
...
def check_mapped(sample,flagstat,min_mapped_reads=500) {
    ...
    if (mapped < min_mapped_reads.toInteger()) {
        ...
        //log.info "-${c_green}           Passed mapped read threshold > bwa ($sample)   >> ${mapped} <<${c_reset}"
...
if (params.protocol != 'amplicon') {
    ...
    process IVAR_TRIM {
        ...
        offset_trim_arg = params.ivar_trim_offset ? "-x ${params.ivar_trim_offset}" : ''
        prefix = "${sample}.trim"
        """
...
process VARSCAN2_CONSENSUS {
    ...
    mosdepth \\
        ${prefix} \\
        ${bam[0]}
    zcat ${prefix}.per-base.bed.gz | awk '\$4 < $params.min_coverage' > ${prefix}.lowcov.bed
...
/*
 * STEP 5.7.2.1: Generate consensus sequence for Ivar using the VCF file, not Ivar consensus
 */
process IVAR_CONSENSUS {
    ...
    input:
    tuple val(sample), val(single_end), path(bam), path(vcf) from ch_markdup_bam_ivar_consensus.join(ch_ivar_highfreq_consensus, by: [0,1])
    path fasta from ch_fasta

    output:
    tuple val(sample), val(single_end), path("*.consensus.fa") into ch_ivar_consensus
    path "*.{txt,tsv,pdf}"
    ...
    bedtools maskfasta \\
        ...
        -bed ${prefix}.mask.bed \\
        -fo ${index_base}.ref.masked.fa
    cat ${index_base}.ref.masked.fa | bcftools consensus ${vcf[0]} > ${prefix}.consensus.fa
    header=\$(head -n 1 ${prefix}.consensus.fa | sed 's/>//g')
...
process BCFTOOLS_CONSENSUS {
    ...
    mosdepth \\
        ${sample} \\
        ${bam[0]}
    zcat ${sample}.per-base.bed.gz | awk '\$4 < $params.min_coverage' > ${sample}.lowcov.bed
...
process MULTIQC {
    ...
    path ('bwa/log/*') from ch_bwa_mqc.collect().ifEmpty([])
    path ('bwa/flagstat/*') from ch_sort_bam_flagstat_mqc.collect().ifEmpty([])
